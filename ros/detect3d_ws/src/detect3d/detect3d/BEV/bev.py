...
# Produces two identical stub BEV images from the point cloud
def bev_from_pcl(points_xyzr):
    """
    points_xyzr : (N,4) numpy array [x,y,z,r]
    Returns two uint8 RGB images (front_bev, back_bev) sized 608×608×3.

    ✱ Replace this with your real BEV-builder if you have one ✱
    """
    H, W = 608, 608
    bev_front = np.zeros((H, W, 3), dtype=np.uint8)
    bev_back  = np.zeros_like(bev_front)

    # Project each point onto front BEV (simple occupancy)
    # front view: +X forward, +Y left
    mask_front = points_xyzr[:, 0] > 0
    pts_f = points_xyzr[mask_front]
    if pts_f.size:
        x_idx = np.clip((pts_f[:, 0] / 50.0 * H).astype(int), 0, H - 1)
        y_idx = np.clip(((pts_f[:, 1] + 25.0) / 50.0 * W).astype(int), 0, W - 1)
        bev_front[x_idx, y_idx] = (0, 255, 0)  # green dots

    # back view: -X backward, mirror left/right
    mask_back = points_xyzr[:, 0] <= 0
    pts_b = points_xyzr[mask_back]
    if pts_b.size:
        x_idx = np.clip(((-pts_b[:, 0]) / 50.0 * H).astype(int), 0, H - 1)
        y_idx = np.clip(((pts_b[:, 1] + 25.0) / 50.0 * W).astype(int), 0, W - 1)
        bev_back[x_idx, y_idx] = (0, 0, 255)  # blue dots

    return bev_front, bev_back
